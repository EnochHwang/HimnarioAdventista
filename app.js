//////////////////////////////////////////////////////////////////////////////////
// Himnario Adventista Progressive Web App (PWA)
// Copyright 2026 Enoch Hwang

const APP_NAME = "Himnario_Adventista";
...
function handleMoreAction(action) {
  ...
  switch(action) {
    ...
    case 'updates':
      ...
      showToast("Aplicación actualizada");   // App updated
      closeMoreMenu();
      break;
    ...
    case 'settings':
      showToast("Configuraciones próximamente");  // Settings coming soon
      break;
...
swiperWrapper.addEventListener('pointerdown', (e) => {
  ...
  longPressTimer = setTimeout(() => {    
    ...
    if (songname) {
      ...
      console.error("Could not find song title at index:", index);
    }
...
addBookmarkMenuOverlay.addEventListener("click", (e) => {
  if (e.target.tagName === "BUTTON") {
    ...
    if (folder !== "Cancel") {
      ...
      const s = folder.replace("Folder", "carpeta");
      showToast(`${formattedSongname} añadido a la ${s}`); // added to folder 1 añadido a la carpeta 1
    }
...
function handleFolderLongPress(folderId) {
  if (folderId === 4) {
    showToast("No se pueden eliminar todos los elementos de Mis canciones."); // Cannot delete all items in My Songs
    return; // can't delete My Songs folder
  }
  
  if (currentBookmarkFolder === "Folder " + folderId) {  // Folder
    const bookmarks = JSON.parse(localStorage.getItem(APP_NAME+"_bookmarks") || "{}");
    ...
    if (bookmarks[currentBookmarkFolder] && bookmarks[currentBookmarkFolder].length > 0) {
      ...
      document.getElementById('confirm-message').innerText = `¿Borrar la carpeta ${folderId}?`;  // Clear Folder 1? ¿Borrar la carpeta 1?
      ...
      document.getElementById('confirmYesBtn').onclick = () => {  // YES to clear button clicked
        ...
        //showToast("Carpeta"+folderId+"despejada");  // Folder 1 cleared Carpeta 1 despejada
      };
      ...
      showToast("No hay elementos para borrar"); // No items to clear
    }
...
function createBookmarkList() {
  ...
  if (currentBookmarkFolderItems.length === 0) {
    if (currentBookmarkFolder == "Folder 4") {
      bookmarkList.innerHTML = `<div style="padding: 20px; text-align: center; color: #888;">La función Mis canciones estará disponible próximamente</div>`;
    } else {
      bookmarkList.innerHTML = `<div style="padding: 20px; text-align: center; color: #888;">Mantenga presionada la canción o el índice para agregar un marcador</div>`;  // Long press song or index to add bookmark
    }
    return;
  }
...
function renderSearchList(songs) {
  ...
  if (songs.length === 0) {
    searchList.innerHTML = '<div style="padding:20px; color:#888; text-align:center;">No se encontraron canciones coincidentes</div>'; // No matching songs found
    return;
  }
...
function openMusicPlayer() {
  ...
  if (!playing) {
    ...
    audioFilename.textContent = "Cargando música...";
    ...
    audio.onerror = () => {
      audioFilenameDiv.textContent = "No hay archivo de audio";  // No audio file
    };
...
function stopAudio() {
  ...
  if (audioFilename.textContent === "Sin archivo de audio") {  // No audio file
    playBtn.disabled = true;
  } else {
    playBtn.disabled = false;
  }
  ...
  if (input.files && input.files.length > 0) {  // check if there's actually a file selected
    ...
    if (input.files.length > 1) {
      ...
      */
      ...
      showToast(`Importando ${input.files.length} canciones...`);  // Importing n songs...
      await saveMultipleFiles(input.files);
...
function doneEditMySongs() {
  ...
  try {
    ...
    if (mySongs[filename] && filename !== originalEditName) {
      showToast("El nombre ya existe. Por favor, elija un nombre diferente.");  // Name already exists. Please choose a different name
      return; // Don't save
    }
    ...
    showToast("El almacenamiento está lleno."); // Storage is full
    return; 
  }
...
//    }
  ...
  try {
    ...
    showToast(`${fileArray.length} Canciones importadas a Mis Canciones`);  // songs imported to My Songs
  } catch (e) {
    showToast("El almacenamiento está lleno."); // Storage is full
  }
}
...
function revealUpdateMenuItem() {
  ...
  if (updateItem) {
    ...
    // Show update alert
    setTimeout(() => {
      showToast("Nuevas actualizaciones disponibles"); // New updates available
...
async function registerServiceWorker() {
  if ('serviceWorker' in navigator) {
    ...
    navigator.serviceWorker.addEventListener('controllerchange', function () {
      ...
      // the following time delay is only for debugging to see the console log
      // setTimeout(adjustSearchListHeight, 5000);  
